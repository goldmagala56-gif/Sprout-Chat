...
export default function ChatWindow({ conversation, userId, onBack, onDelete }) {
  ...
  const subtitle = conversation.online 
    ? 'Online' 
    : isGroup 
      ? `${conversation.members?.length || 0} members` 
      : formatLastSeen(conversation.otherUser?.last_seen);
    ...
    <div className="flex flex-col h-full w-full" style={{ backgroundColor: '#f0f2f5' }}>
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <button 
            onClick={onBack}
            className="md:hidden p-1 -ml-1 rounded-full hover:bg-black/5 flex-shrink-0"
          >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <button className="p-2 rounded-full hover:bg-black/5 transition-colors hidden sm:block">
      ...
      {/* Composer */}
      <Composer onSend={sendMessage} disabled={!userId} />
    </div>
  );
}
