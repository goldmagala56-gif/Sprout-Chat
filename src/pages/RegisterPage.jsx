        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2 rounded-xl px-3 py-3" style={{ backgroundColor: COLORS.bg, border: `1px solid ${COLORS.panelBorder}` }}>
              ...
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name" className="w-full bg-transparent outline-none text-sm" style={{ color: COLORS.text }} required />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 rounded-xl px-3 py-3" style={{ backgroundColor: COLORS.bg, border: `1px solid ${COLORS.panelBorder}` }}>
              ...
              <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 6 characters" className="w-full bg-transparent outline-none text-sm" style={{ color: COLORS.text }} required />
              <button type="button" onClick={() => setShowPass(!showPass)}><Eye size={16} color={COLORS.textMuted} /></button>
            </div>
          </div>
